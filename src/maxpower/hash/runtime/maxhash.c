		...
		size_t max_size_bits, size_t max_size_bytes, bool is_value)
{
	...
	for (size_t i = start; i < item_size_bytes; i++) {
		...
		if (bad) {
			...
			fprintf(stderr, "Error: %s has bits set beyond the maximum %s size of the hash table (%zu "
					"bits): byte %zu has the value 0x%x.\n", type, type, max_size_bits, i, val);
			return MAXHASH_ERR_ERR;
		}
	}
	size_t copy_size = item_size_bytes < max_size_bytes ? item_size_bytes : max_size_bytes;
...
void maxhash_debug_print(const maxhash_table_t *table, const char *fmt, ...)
{
	...
	snprintf(time_string, sizeof(time_string), "[%04d-%02d-%02d %02d:%02d:%02d.%06ld] ",
			1900 + utc_time.tm_year, 1 + utc_time.tm_mon, utc_time.tm_mday, utc_time.tm_hour,
			utc_time.tm_min, utc_time.tm_sec, current_time.tv_usec);
...
uint32_t maxhash_function_jenkins(const void *data, size_t data_len, uint32_t hashparam, size_t chunk_width)
{
	bool debug = false;
	...
	for (size_t chunk = 0; chunk < data_len; chunk += chunk_width) {
		for (size_t octet = 0; octet < chunk_width; octet++)
			hash += chunk + octet < data_len ? ((uint8_t *)data)[chunk + octet] << (octet * 8) : 0;
		hash += hash << 10;
		hash ^= hash >> 6;
		if (debug)
			printf("hash: %x\n", hash);
...
void hash_print_entry(const maxhash_internal_table_t *itable, const maxhash_entry_t *entry)
{
	...
	if (entry->flags[FLAG_VALID] || entry->flags[FLAG_PERFECT_DIRECT]) {
		printf(" (");
...
void hash_print(const maxhash_internal_table_t *itable, bool sparse)
{
	...
	for (size_t entry_id = 0; entry_id < itable->iparams.num_buckets; entry_id++) {
...
maxhash_err_t maxhash_internal_table_init(maxhash_internal_table_t *itable,
		const maxhash_table_t *table,
		const maxhash_internal_table_params_t *itable_params,
		const char *itable_name)
{
	memcpy(&itable->iparams, itable_params, sizeof(maxhash_internal_table_params_t));

	itable->iparams.width_bytes = (itable->iparams.width_bits + 7) / 8;
	itable->table = table;
	itable->buckets = calloc(itable_params->num_buckets, sizeof(maxhash_bucket_t));
	...
	if (strlen(table->tparams.hash_table_name) == 0) {
		strncpy(itable->iparams.name, itable_name, sizeof(itable->iparams.name));
	} else {
		snprintf(itable->iparams.name, sizeof(itable->iparams.name), "%s_%s",
				table->tparams.hash_table_name, itable_name);
	}
		...
		const maxhash_table_params_t *tparams)
{
	...
	*table = calloc(1, sizeof(maxhash_table_t));
	...
	table_p->load_buffer_select = 0;

	const maxhash_internal_table_params_t *intermediate_params = &tparams->intermediate;
	const maxhash_internal_table_params_t *values_params = &tparams->values;
	...
	err |= maxhash_internal_table_init(&table_p->sw, table_p, &sw_params, "Software");
	err |= maxhash_internal_table_init(&table_p->recent, table_p, &sw_params, "Recent");
	err |= maxhash_internal_table_init(&table_p->intermediate, table_p, intermediate_params, "HashParams");
	err |= maxhash_internal_table_init(&table_p->values, table_p, values_params, "Values");
...
maxhash_err_t maxhash_table_params_init(maxhash_table_params_t **tparams)
{
...
maxhash_err_t maxhash_table_params_free(maxhash_table_params_t *tparams)
{
	free(tparams);
	return MAXHASH_ERR_OK;
}

maxhash_err_t maxhash_table_params_set_size(maxhash_table_params_t *tparams, size_t size)
{
	tparams->values.num_buckets = size;
	return MAXHASH_ERR_OK;
}

maxhash_err_t maxhash_table_params_set_key_width_bits(maxhash_table_params_t *tparams, size_t key_width_bits)
{
	tparams->key_width_bits = key_width_bits;
	return MAXHASH_ERR_OK;
}

maxhash_err_t maxhash_table_params_set_value_width_bits(maxhash_table_params_t *tparams, size_t value_width_bits)
{
...
maxhash_err_t maxhash_sw_table_init(maxhash_table_t **table, const maxhash_table_params_t *tparams)
{
	...
	strncpy(params_copy.hash_table_name, tparams->hash_table_name, sizeof(params_copy.hash_table_name));
...
maxhash_err_t maxhash_check_presence(maxhash_engine_state_t *es, const char *kernel_name, const char *hash_table_name)
{
		...
		maxhash_engine_state_t *es, const char *full_hash_name, const char *mem_name)
{
	...
	if (!strcmp(mem_type, "FMem")) {
		...
		snprintf(cmd_stream_name, sizeof(cmd_stream_name), "%s_%s_LMemWriteCPUCtrl",
				full_hash_name, mem_name);
		snprintf(write_cpu_stream_name, sizeof(write_cpu_stream_name), "%s_%s_LMemWriteCPUData",
				full_hash_name, mem_name);
		snprintf(write_stream_name, sizeof(write_stream_name), "%s_%s_LMemWriteCard",
				full_hash_name, mem_name);
		mem_backing_params->lmem_handle = lmem_init_cpu_access_named(es->maxfile, es->engine,
				cmd_stream_name, write_cpu_stream_name, NULL, write_stream_name, NULL, buf);
	} else {
	...
	if (width_bits <= 0) {
		fprintf(stderr, "Error: table entry width in hardware hash table (%d) is invalid.\n", width_bits);
		return MAXHASH_ERR_ERR;
	}
	...
	if (num_buckets <= 0) {
		fprintf(stderr, "Error: number of buckets in hardware hash table (%d) is invalid.\n", num_buckets);
		return MAXHASH_ERR_ERR;
	}
...
maxhash_err_t maxhash_get_hw_params(maxhash_table_params_t *tparams, maxhash_engine_state_t *es,
		const char *kernel_name, const char *hash_table_name)
{
	...
	if (key_width_bits <= 0) {
		fprintf(stderr, "Error: key width in hardware hash table (%d) is invalid.\n", key_width_bits);
		return MAXHASH_ERR_ERR;
	}

	if (jenkins_chunk_width_bits <= 0 || jenkins_chunk_width_bits % 8 != 0) {
		fprintf(stderr, "Error: Jenkins chunk width in hardware hash table (%d) is invalid.\n",
...
maxhash_err_t maxhash_hw_table_init(maxhash_table_t **table, const char *kernel_name, const char *hash_table_name,
		maxhash_engine_state_t *es)
{
	...
	strncpy(tparams.hash_table_name, hash_table_name, sizeof(tparams.hash_table_name));

	err = maxhash_get_hw_params(&tparams, es, kernel_name, hash_table_name);
	if (err != MAXHASH_ERR_OK) {
		fprintf(stderr, "Error: failed to access required MaxHash information in the MaxFile.\n");
		return err;
	}

	err = maxhash_internal_sw_init(table, &tparams);
	if (err != MAXHASH_ERR_OK) {
		fprintf(stderr, "Error: failed to initialize MaxHash software layer.\n");
		return err;
	}
...
maxhash_err_t maxhash_internal_clear_bucket(maxhash_internal_table_t *itable, size_t bucket_id)
{
...
{
	for (size_t bucket_id = 0; bucket_id < itable->iparams.num_buckets; bucket_id++)
...
maxhash_err_t maxhash_contains_in_bucket(const maxhash_internal_table_t *itable,
		bool *present, const void *key, size_t bucket_id)
{
	*present = false;

	for (maxhash_entry_t *entry = itable->buckets[bucket_id].entry_list; entry; entry = entry->next) {
...
maxhash_err_t maxhash_internal_get_bucket_id(const maxhash_internal_table_t *itable, size_t *bucket_id,
		const void *key, size_t hashparam)
{
	*bucket_id = maxhash_function_jenkins(key, itable->table->tparams.key_width_bytes,
			hashparam, itable->table->tparams.jenkins_chunk_width_bytes)
			% itable->iparams.num_buckets;
	return MAXHASH_ERR_OK;
}
...
maxhash_err_t maxhash_contains(const maxhash_table_t *table, bool *present, const void *key, size_t key_len)
{
...
maxhash_err_t maxhash_put(maxhash_table_t *table,
...
maxhash_err_t maxhash_internal_get_entry_in_bucket(const maxhash_internal_table_t *itable, bool check_key,
...
maxhash_err_t maxhash_internal_get_entry(const maxhash_internal_table_t *itable, bool check_key,
		const void *key, maxhash_entry_t *entry)
{
	size_t bucket_id;
	maxhash_internal_get_bucket_id(itable, &bucket_id, key, 0);
	return maxhash_internal_get_entry_in_bucket(itable, check_key, key, entry, bucket_id);
}



maxhash_err_t maxhash_internal_get(const maxhash_internal_table_t *itable, bool check_key,
...
maxhash_err_t maxhash_internal_get_flag(const maxhash_entry_t *entry, int flag, bool *value)
{
		...
		const void *key, size_t *index)
{
	...
	*index = 0;
	...
	maxhash_entry_t entry;
	err |= maxhash_internal_get_entry(&table->intermediate, false, key, &entry);
	...
	err |= maxhash_internal_get_flag(&entry, FLAG_PERFECT_DIRECT, &perfect_direct);
		...
		const void *key, void *value, bool *valid)
{
	...
	maxhash_entry_t entry;
	err |= maxhash_internal_get_entry_in_bucket(&table->values, true, key, &entry, index);
...
maxhash_err_t maxhash_get(const maxhash_table_t *table, const void *key, size_t key_len, void *value)
{
	PAD_KEY(&table->tparams, key, key_len);
		...
		const void *key, size_t bucket_id)
{
	...
	/* Empty list. */
	if (!cur) {
		fprintf(stderr, "Error: attempted to remove an entry that is not present in the \"%s\" hash table.\n",
				itable->iparams.name);
		return MAXHASH_ERR_ERR;
	}
	...
	fprintf(stderr, "Error: attempted to remove an entry that is not present in the requested hash table.\n");
	return MAXHASH_ERR_ERR;
}



maxhash_err_t maxhash_internal_remove(maxhash_internal_table_t *itable, const void *key)
{
	size_t bucket_id;
	maxhash_internal_get_bucket_id(itable, &bucket_id, key, 0);
...
maxhash_err_t maxhash_perfect_values_remove(maxhash_table_t *table, const void *key)
{
	maxhash_err_t err = MAXHASH_ERR_OK;
...
maxhash_err_t maxhash_remove(maxhash_table_t *table, const void *key, size_t key_len)
{
	PAD_KEY(&table->tparams, key, key_len);
...
maxhash_err_t write_mem(const maxhash_internal_table_t *itable, const char *buf_name, void *buf, size_t mem_size_bytes)
{
	const maxhash_table_params_t *tparams = &itable->table->tparams;
	...
	if (itable->iparams.mem_type == MAXHASH_MEM_TYPE_FMEM) {
		maxhash_write_fmem(tparams->engine_state, tparams->kernel_name, buf_name,
				itable->table->load_buffer_select * mem_size_bytes / sizeof(uint64_t),
				buf, mem_size_bytes);
	} else if (itable->iparams.mem_type == MAXHASH_MEM_TYPE_DEEP_FMEM) {
		maxhash_write_deep_fmem(tparams->engine_state, tparams->kernel_name, buf_name,
				buf, mem_size_bytes);
	} else if (itable->iparams.mem_type == MAXHASH_MEM_TYPE_LMEM) {
		size_t lmem_burst_size_bytes = lmem_get_burst_size_bytes(itable->iparams.lmem_handle);
		size_t mem_size_bursts = (mem_size_bytes + lmem_burst_size_bytes - 1) / lmem_burst_size_bytes;
		lmem_write(itable->iparams.lmem_handle,
				itable->iparams.base_address_bursts +
				itable->table->load_buffer_select * mem_size_bursts,
				buf, mem_size_bursts);
	} else {
		fprintf(stderr, "Error: hash table memory type is invalid.\n");
		return MAXHASH_ERR_ERR;
...
static size_t write_entry(void *dest, size_t offset_bits, const void *src, size_t size_bits)
{
...
maxhash_err_t write_table_data(const maxhash_internal_table_t *itable, bool has_direct_flag)
{
	maxhash_err_t err = MAXHASH_ERR_OK;
	const maxhash_table_params_t *tparams = &itable->table->tparams;

	size_t deep_fmem_id_bits = itable->iparams.mem_type == MAXHASH_MEM_TYPE_DEEP_FMEM ? DEEP_FMEM_ID_BITS : 0;

	size_t num_flags = has_direct_flag ? NUM_ENTRY_FLAGS : NUM_ENTRY_FLAGS - 1;

	size_t mem_entry_size_bits = deep_fmem_id_bits + num_flags + itable->iparams.width_bits;
	...
	if (itable->iparams.mem_type == MAXHASH_MEM_TYPE_FMEM) {
		...
		burst_size_bytes = lmem_get_burst_size_bytes(itable->iparams.lmem_handle);
	...
	if (burst_size_bytes < PCIE_WIDTH) {
		...
		/* Round burst size up to a multiple of the PCIe interface size, if not already. */
		burst_size_bytes = ((burst_size_bytes + PCIE_WIDTH - 1) / PCIE_WIDTH) * PCIE_WIDTH;
	}
	...
	size_t num_bursts = (tparams->max_bucket_entries * itable->iparams.num_buckets + entries_per_burst - 1)
			/ entries_per_burst;
	...
	for (size_t bucket_id = 0; bucket_id < itable->iparams.num_buckets; bucket_id++) {
		...
		for (size_t bucket_entry = 0; bucket_entry < tparams->max_bucket_entries; bucket_entry++) {
			size_t entry_id = bucket_entry * itable->iparams.num_buckets + bucket_id;

			size_t burst = entry_id / entries_per_burst;
			size_t entry_in_burst = entry_id % entries_per_burst;
			size_t offset_bits = (burst * burst_size_bytes + entry_in_burst * mem_entry_size_bytes) * 8;

			if (offset_bits >= mem_size * 8) {
				fprintf(stderr, "Error: attempted to write to an invalid memory location.\n");
				return MAXHASH_ERR_ERR;
			}
			...
			if (itable->iparams.mem_type == MAXHASH_MEM_TYPE_DEEP_FMEM) {
				...
				offset_bits += write_entry(mem_contents, offset_bits, &deep_fmem_id,
						DEEP_FMEM_ID_BITS);
			}
			...
			if (e && e->flags[FLAG_VALID]) {
				...
				if (has_direct_flag)
					flags |= e->flags[FLAG_PERFECT_DIRECT] << FLAG_PERFECT_DIRECT;

				offset_bits += write_entry(mem_contents, offset_bits, &flags, num_flags);
				...
				offset_bits += write_entry(mem_contents, offset_bits,
						e->value, itable->iparams.width_bits);
	...
	if (tparams->max_bucket_entries > 1) {
		for (size_t mem_id = 0; mem_id < tparams->max_bucket_entries; mem_id++) {
			...
			snprintf(name_buf, sizeof(name_buf), "%s_Buckets%zu", itable->iparams.name, mem_id);
			err |= write_mem(itable, name_buf, mem_contents, mem_size);
		}
	} else {
		err |= write_mem(itable, itable->iparams.name, mem_contents, mem_size);
	}
...
{
	char name_buf[NAME_BUF_LEN] = {0};
	snprintf(name_buf, sizeof(name_buf), "%s_BufferSelect", table->tparams.hash_table_name);

	uint64_t load_buffer_select = table->load_buffer_select;

	maxhash_write_fmem(table->tparams.engine_state, table->tparams.kernel_name, name_buf, 0,
			&load_buffer_select, 1);
...
maxhash_err_t maxhash_commit(maxhash_table_t *table)
{
	...
	for (size_t i = 0; i < table->intermediate.iparams.num_buckets; i++)
		assert((table->values.buckets[i].num_keys == 0 && !table->values.buckets[i].entry_list)
				|| (table->values.buckets[i].num_keys > 0 && table->values.buckets[i].entry_list));
...
maxhash_err_t maxhash_putall(maxhash_table_t *destination, const maxhash_table_t *source)
{
	for (size_t bucket_id = 0; bucket_id < source->sw.iparams.num_buckets; bucket_id++) {
		maxhash_bucket_t *bucket = &source->sw.buckets[bucket_id];
		for (maxhash_entry_t *e = bucket->entry_list; e; e = e->next) {
			maxhash_err_t err = maxhash_internal_put(&destination->sw, e->key, e->value);
			if (err != MAXHASH_ERR_OK)
				return err;
		}
	}
...
maxhash_err_t maxhash_perfect_get_capacity(const maxhash_table_t *table, size_t *capacity)
{
	if (table->tparams.max_bucket_entries == 1) {
...
maxhash_err_t maxhash_get_key_width(const maxhash_table_t *table, size_t *key_width)
{
...
maxhash_err_t maxhash_get_value_width(const maxhash_table_t *table, size_t *value_width)
{
...
maxhash_err_t maxhash_get_keys(const maxhash_table_t *table, void *keys)
{
	...
	size_t entry_id = 0;
	for (size_t bucket_id = 0; bucket_id < table->sw.iparams.num_buckets; bucket_id++) {
...
maxhash_err_t maxhash_entry_iterator_init(maxhash_entry_iterator_t **iterator, const maxhash_table_t *table)
{
...
{
	/* Reuse next entry that was found incidentally in maxhash_entry_iterator_has_next. */
	if (iterator->next) {
	...
	/* User shouldn't call next() without checking has_next(), but if they do, handle this correctly. */
	bool has_next;
...
{
	/* Look for next entry, and if found, save it in iterator->next for use by maxhash_entry_iterator_next. */
	...
	*has_next = true;
	...
	iterator->next = NULL;
	while (!iterator->next && iterator->bucket_id < iterator->itable->iparams.num_buckets) {
...
maxhash_err_t maxhash_entry_iterator_get_key(maxhash_entry_iterator_t *iterator, const void **key)
{
...
maxhash_err_t maxhash_entry_iterator_get_value(maxhash_entry_iterator_t *iterator, const void **value)
{
...
maxhash_err_t maxhash_perfect_create(maxhash_table_t *table)
{
	...
	if (enable_incremental_puts) {
		...
		if (table->tparams.debug) {
			maxhash_print_sparse(&table->intermediate);
			maxhash_print_sparse(&table->values);
		}

		for (size_t bucket_id = 0; bucket_id < table->recent.iparams.num_buckets; bucket_id++) {
			maxhash_bucket_t *recent_bucket = &table->recent.buckets[bucket_id];
			maxhash_bucket_t *sw_bucket = &table->sw.buckets[bucket_id];
			if (recent_bucket->num_keys > 0 && sw_bucket->num_keys > recent_bucket->num_keys) {
				for (maxhash_entry_t *entry = sw_bucket->entry_list; entry; entry = entry->next) {
					bool is_new_entry;
					...
					if (!is_new_entry) {
						...
						err |= maxhash_internal_put(&table->recent, entry->key, entry->value);
						if (err != MAXHASH_ERR_OK) {
				...
				err |= maxhash_internal_clear_bucket(&table->intermediate, bucket_id);
			}
		}

		if (table->tparams.debug)
			maxhash_print_sparse(&table->recent);

		printf("Incremental puts: need to move %zu entries in the values table.\n", moves);
	} else {
	...
	/* Copy bucket list and sort in-place in reverse order of the number of collisions. */
	size_t buckets_size = source_itable->iparams.num_buckets * sizeof(maxhash_bucket_t);
	maxhash_bucket_t *sorted_buckets = malloc(buckets_size);
	memcpy(sorted_buckets, source_itable->buckets, buckets_size);
	qsort(sorted_buckets, source_itable->iparams.num_buckets, sizeof(maxhash_bucket_t), compare_bucket_num_keys);

	size_t bucket_id = 0;
	maxhash_bucket_t *bucket = &sorted_buckets[bucket_id];

	/* Temporary array in which to store the hashes of all of the entries in a bucket. */
	...
	const char *columns[] = {"Collisions", "Buckets", "Max. attempts", "Avg. attempts", "Num. hashes"};
	...
	while (bucket_id < table->intermediate.iparams.num_buckets && bucket->num_keys >= 2) {
		uint32_t d = 0;
		bool found = false;

		/* Search for a value of 'd' that assigns keys to unique slots. */
		while (!found) {
			if ((size_t)d >= (1ul << table->intermediate.iparams.width_bytes * 8) - 1) {
				fprintf(stderr, "Error: failed to find a hash parameter (d: %d, bucket: %zd).\n",
						d, bucket_id);
				return MAXHASH_ERR_ERR;
			}
			...
			while (entry && found) {
				...
				if (found) {
					if (entry_id >= new_hashes_size) {
						fprintf(stderr, "Error: this implementation only supports up to %zu "
								"collisions per hash bucket.\n", new_hashes_size);
						return MAXHASH_ERR_ERR;
					}
		...
		entry_id = 0;
		for (maxhash_entry_t *entry = bucket->entry_list; entry; entry = entry->next)
			maxhash_internal_put_in_bucket(&table->values, entry->key, entry->value,
					new_hashes[entry_id++]);

		maxhash_internal_put(&table->intermediate, bucket->entry_list->key, &d);
		...
		if (bucket->num_keys != prev_num_keys) {
			double average_searches = (double)parameter_sum / (bucket_id - prev_bucket_id);
	...
	maxhash_bucket_t *values_bucket = &table->values.buckets[values_bucket_id];

	/* Iterate through the rest of the occupied buckets, assigning entries directly to buckets in the new table
	 * rather than searching for a hash parameter. */
	while (bucket_id < table->intermediate.iparams.num_buckets &&
		...
		if (!values_bucket->num_keys) {
			err |= maxhash_internal_put(&table->intermediate, bucket->entry_list->key, &values_bucket_id);
			err |= maxhash_internal_set_entry_flag(&table->intermediate, bucket->entry_list->key,
					FLAG_PERFECT_DIRECT, true);
			err |= maxhash_internal_put_in_bucket(&table->values, bucket->entry_list->key,
					bucket->entry_list->value, values_bucket_id);
			bucket = &sorted_buckets[++bucket_id];
		}
