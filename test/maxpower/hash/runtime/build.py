...
MAXFILE = 'MaxHashTest.max'
DESIGN_NAME = MAXFILE.replace('.max', '')
sources = ['maxhashtest.c']
target = 'maxhashtest'
includes = ['-I%s/src/maxpower/lmem/cpuaccess/runtime/' % (MAXPOWERDIR),
			'-I%s/src/maxpower/hash/runtime/include/' % (MAXPOWERDIR) ]
MAXPOWER_LIBS = ['-L%s/src/maxpower/hash/runtime' % (MAXPOWERDIR), '-lmaxhash-slic',
				 '-L%s/src/maxpower/lmem/cpuaccess/runtime/' % (MAXPOWERDIR), '-llmem_cpu_access',
				 '-L%s/src/maxpower/lmem/runtime/' % (MAXPOWERDIR), '-llmem' ]
